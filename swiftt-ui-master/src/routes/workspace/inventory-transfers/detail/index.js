...
@connect(({ inventoryTransfer, loading }) => ({
  inventoryTransfer,
...
class InventoryTransferViewWrapper extends PureComponent {
  ...
  componentDidMount() {
    ...
    const match = pathToRegexp('/workspace/inventory-transfer/view/:id').exec(location.pathname);
    if (match) {
  ...
  render() {
    ...
    const inventoryTransferFormProps = {
      ...
      onSave(values) {
        ...
        dispatch({ type: 'inventoryTransfer/save', payload: payload });
      },
      onSubmit(values) {
        dispatch({ type: 'inventoryTransfer/submit', payload: { id: data.id, fromWorkspace: true } });
      },
      onOrderFulfilled(items) {
        dispatch({ type: 'inventoryTransfer/fulfil', payload: { id: data.id, items: items, fromWorkspace: true } });
      },
      onReceived(items) {
        dispatch({ type: 'inventoryTransfer/receive', payload: { id: data.id, items: items, fromWorkspace: true } });
      },
      onReject() {
        dispatch({ type: 'inventoryTransfer/reject', payload: { id: data.id, fromWorkspace: true } });
      },
    };
