...
import { fetchCashiersCollectionSummary } from '../../../../services/accounting/reports';

const displayDateFormat = 'DD MMM YYYY';
const dateFormat = 'YYYY-MM-DD HH:mm:ss';
const { RangePicker } = DatePicker;
let cumulativeTotal = 0;
class CashiersCollectionSummary extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      payeeAccount: {},
  ...
  getReportData = (payload) => {
    ...
    fetchCashiersCollectionSummary({ payload }).then((response) => {
      this.setState({
        loading: false,
        dataSource: response,
      });
    ...
    const pageHeaderContent = (
      <div className={styles.pageHeaderContent}>
        ...
        <div className={styles.content}>
          <div className={styles.contentTitle}>Casher Collection Summary</div>
        </div>
      </div>
    );
    const dateFilter = (
    ...
    return (
      ...
      >
        ...
        <Table
          ...
          bordered
          pagination={false}
          rowKey={record => record.journalReferenceId}
          footer={() => {
              return (
                <table style={{ marginTop: 0, border: "none"}} className="">
                  <tbody className="ant-table-tbody">
                    <tr className="ant-table-row">
                      <td align="right" style={{ fontWeight: 600, fontSize: 13, color: "#fff" }} colSpan={4}>
                        Total
                        </td>
                      <td align="right" style={{ fontWeight: 600, fontSize: 13, color: "#fff", border: "none"   }} width="100px">
...
export default CashiersCollectionSummary;
