    ...
    const columns = [
      {
        ...
        title: 'Customer / Scheme',
        dataIndex: 'scheme.name',
        key: 'scheme.name',
        render: (text, record) => {
          if (record.scheme) {
            return <Ellipsis lines={1}>{record.scheme.contact.name} / {record.scheme.name}</Ellipsis>;
          }
        },
      }, {
        title: 'Payment Type',
        ...
        render: (text) => {
          if (text === 'CASH_PREPAY') {
            return (<span>Cash PrePay</span>);
          } else if (text === 'CASH_POSTPAY') {
            return (<span>Cash PostPay</span>);
          } else if (text === 'CREDIT') {
        ...
        dataIndex: 'priceList',
        key: 'priceList',
        render: (text, record) => <Ellipsis lines={1}>{text ? record.priceList.name : 'Not specified'}</Ellipsis>,
      }, {
        title: '',
        key: 'operation',
        width: 100,
