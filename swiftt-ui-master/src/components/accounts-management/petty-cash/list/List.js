...
class JournalReferenceList extends PureComponent {
  constructor(props) {
    ...
    const { current } = this.props.pettyCash.pagination;
  ...
  render() {
    const { list, pagination, success, loading } = this.props.pettyCash;
    ...
    const calculateBillItemsAggregate = (dataset) => {
      ...
      cumulativeTotal = dataset.reduce((tot, arr) => {
        return tot + cumulativeTotal + arr.totalAmount;
      }, 0);
                  ...
                  <tbody className="ant-table-tbody">
                    <tr className="ant-table-row">
                      ...
                      <td align="right" style={{ fontWeight: 800, fontSize: 15, color: "#fff"  }} width="15%">
                        <span>{numeral(calculateBillItemsAggregate(list)).format('0,0.00')}</span>
                      </td>
                    </tr>
                  </tbody>
                </table>
