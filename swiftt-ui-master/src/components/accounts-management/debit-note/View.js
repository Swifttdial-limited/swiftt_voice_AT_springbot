...
function VendorBillView({
  ...
  if (creditNote.transactionLines.length) {
    let num = 0;
    let amount = 0;
    creditNote.transactionLines.forEach((item) => {
      num += Number(item.quantity);
      amount += Number(item.cost);
      item.lineTotal = item.quantity * item.cost;
    });
    transactionLines = creditNote.transactionLines.concat({
      id: ' Total ',
      num,
      amount,
    });
  ...
  const renderContent = (value, row, index) => {
    ...
    if (index === creditNote.transactionLines.length) {
      obj.props.colSpan = 0;
    }
    return obj;
  };
  ...
  function handleGeneratePurchaseOrder(e) {
    dispatch({ type: 'creditNote/createPurchaseOrder', payload: { id: creditNote.id } });
  }
  ...
  return (
    <div>
      ...
      <Card>
        ...
        <h4 style={{ textAlign: 'right' }}><strong>VendorBill No:</strong> {creditNote.creditNoteNumber}</h4>
        ...
        <RelatedView creditNote={creditNote} />

        <Row gutter={24}>
          <Col span={12}><h5><strong>Raised By:</strong> {creditNote.requestedBy.fullName}</h5></Col>
          <Col span={6}><h5><strong>Signature:</strong> </h5></Col>
          <Col span={6}><h5><strong>Date:</strong> {moment(creditNote.requestDate).local().format(dateTimeFormat)}</h5></Col>
        </Row>

        <Row gutter={24}>
          <Col span={12}><h5><strong>Approved By:</strong> {creditNote.requestedBy.fullName}</h5></Col>
          <Col span={6}><h5><strong>Signature:</strong> </h5></Col>
          <Col span={6}><h5><strong>Date:</strong> {moment(creditNote.requestDate).local().format(dateTimeFormat)}</h5></Col>
        </Row>
