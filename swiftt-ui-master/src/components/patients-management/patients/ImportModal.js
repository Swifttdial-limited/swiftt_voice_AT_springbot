...
class ImportModal extends PureComponent {
  ...
  downloadFile = () => {
    Ajax.ajax({
      headers,
      url: `${API_URL + api.patientsImport}`,
      method: 'get',
      contentType: 'text/csv',
    }).then(data => {
      fileDownload(data, 'patients-import-template.csv', 'text/csv');
    }).fail((error) => {
      console.log(error)
    });
  }
  ...
  render() {
    ...
    const props = {
      ...
      accept: '.csv',
      headers,
      action: `${API_URL + api.patientsImport}`,
      data: {},
    ...
    return (
      <Modal {...modalOpts}>
        <Alert
          description={(
            <div>
              To get started you'll need to format patients in the correct way. Download a template CSV
