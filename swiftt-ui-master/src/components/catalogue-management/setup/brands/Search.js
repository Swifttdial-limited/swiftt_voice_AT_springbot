...
import SearchBrand from '../../../ui/search';
  ...
  return (
    <Row gutter={24}>
      <Col lg={12} md={12} sm={16} xs={24} style={{ marginBottom: 16 }}>
        <SearchBrand {...searchBrandProps} />
      </Col>
      <Col lg={{ offset: 6, span: 6 }} md={12} sm={8} xs={24} style={{ marginBottom: 16, textAlign: 'right' }}>
        <Authorized authority="CREATE_BRAND">
          <Button type="primary" onClick={onAdd}><Icon type="plus" />New Brand</Button>
        </Authorized>
      </Col>
    </Row>
  );
