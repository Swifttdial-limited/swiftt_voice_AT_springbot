...
import { query } from '../../services/fixed-asset-mgt/assets';

const { Option } = Select;

class FixedAssetSelect extends PureComponent {
  static defaultProps = {
    autoLoad: true,
    disabled: false,
    multiSelect: false,
  ...
  mapSelectedValuesToProduct = (values) => {
    ...
    values.forEach((selectedProduct) => {
      selectedProducts.push({ assetName: selectedProduct.label, id: selectedProduct.key });
    })
  ...
  render() {
    ...
    return (
      <Select
        ...
        labelInValue={true}
        showSearch
        placeholder={multiSelect ? 'Select Asset(s)' : 'Select Asset'}
        mode={multiSelect ? 'multiple' : ''}
        notFoundContent={loading ? <Spin size="small" /> : 'No asset matching search criteria found'}
