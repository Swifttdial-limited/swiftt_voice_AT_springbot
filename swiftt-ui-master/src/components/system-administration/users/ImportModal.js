...
class ImportModal extends PureComponent {
  ...
  downloadFile = () => {
    Ajax.ajax({
      headers,
      url: `${API_URL + api.accessControl.usersImport}`,
      method: 'get',
      contentType: 'text/csv',
    }).then(data => {
      fileDownload(data, 'users-import-template.csv', 'text/csv');
    }).fail((error) => {
      console.log(error)
    });
  }
  ...
  render() {
    ...
    const props = {
      ...
      accept: '.csv',
      headers,
      action: `${API_URL + api.accessControl.usersImport}`,
      data: {},
    ...
    return (
      <Modal {...modalOpts}>
        <Alert
          description={(
            <div>
              To get started you'll need to format users in the correct way. Download a template CSV
