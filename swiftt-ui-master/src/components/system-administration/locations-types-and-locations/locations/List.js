...
class list extends PureComponent {
  ...
  render() {
    ...
    const renderContent = (text, record) => {
      ...
      record.locationTypes.map((locationType) => {
        const locationTypeTag = <Tag key={locationType.id} closable={false}>{locationType.name}</Tag>;
        locationTypeTags.push(locationTypeTag);
      });

      return <span>{locationTypeTags}</span>;
    };
    ...
    const columns = [
      {
        ...
        key: 'operatingTime',
        render: (text, record) => <span>{record.operatingTime ? `${record.operatingTime.startTime} - ${record.operatingTime.endTime}` : 'All day' }</span>,
      }, {
        title: '',
        key: 'operation',
        width: 100,
