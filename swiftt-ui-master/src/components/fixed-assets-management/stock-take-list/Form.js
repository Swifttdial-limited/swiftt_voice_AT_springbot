...
class StockTakeListForm extends PureComponent {
  ...
  static propTypes = {
    ...
    assetStockTakeList: PropTypes.object,
  ...
  render() {
    ...
    const stockTakeItemsTableFormProps = {
      ...
      rowSelectionEnabled: assetStockTakeList.status === 'IN_PROCESS' ? true : false,
    };
        ...
        <Form>
          <Row>
            <Col span={24} style={{ textAlign: 'right', marginBottom: 10 }}>
              <FormItem {...tailFormItemLayout}>
                { assetStockTakeList.id ? (
                  <div>
                    { assetStockTakeList.status === 'INCOMPLETE' && (
                      <div>
                        <Button
                    ...
                    { assetStockTakeList.status === 'IN_PROCESS' && (
                      <div>
                        <Button
                          style={{ marginRight: 10 }}
                          loading={loading}
          ...
          >
            {getFieldDecorator('location', {
              initialValue: assetStockTakeList.location ? assetStockTakeList.location : null,
              rules: [{
                required: true,
                message: 'Receiving location must be specified',
              }],
            })(<LocationSelect
              editValue={assetStockTakeList.location ? assetStockTakeList.location : null}
          ...
          <FormItem {...headerFormItemLayout} label="Stock Taking Date" hasFeedback>
            {getFieldDecorator('stockTakingDate', {
              initialValue: assetStockTakeList.stockTakingDate ? moment(assetStockTakeList.stockTakingDate, dateFormat) : moment(moment().format(dateFormat), dateFormat),
              rules: [
                {
                    type: 'object',
                    required: true,
          ...
          <Collapse bordered={false} defaultActiveKey={['1']}>
            ...
            <div>
              {getFieldDecorator('items', {
                initialValue: assetStockTakeList.id ? assetStockTakeList.items : [],
          ...
          <Row gutter={24}>
            <Col span={15}>
              {assetStockTakeList.id && (
                <FilesView
                  readOnly={false}
                  context={assetStockTakeList.id}
                  contextType="STOCKTAKE_LIST" />
              )}
              <FormItem label="Comment：" hasFeedback>
                {getFieldDecorator('comment', {
                    initialValue: assetStockTakeList.comment,
                 })(<TextArea rows={4} />)}
              </FormItem>
            </Col>
          </Row>
