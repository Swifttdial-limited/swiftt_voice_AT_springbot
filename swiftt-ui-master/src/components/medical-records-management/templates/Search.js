...
import SearchTemplate from '../../ui/search';
  ...
  return (
    <Row gutter={24}>
      <Col lg={12} md={12} sm={16} xs={24} style={{ marginBottom: 16 }}>
        <SearchTemplate {...searchTemplateProps} />
      </Col>
      <Col lg={{ offset: 6, span: 6 }} md={12} sm={8} xs={24} style={{ marginBottom: 16, textAlign: 'right' }}>
        <Link to="/medical-records/template/create">
          <Button type="primary" style={{ marginLeft: 8 }}><Icon type="plus" />New Template</Button>
        </Link>
      </Col>
    </Row>
  );
