...
class ImportModal extends PureComponent {
  ...
  downloadFile = () => {
    Ajax.ajax({
      headers,
      url: `${API_URL + api.diagnosesImport}`,
      method: 'get',
      contentType: 'text/csv',
    }).then(data => {
      fileDownload(data, 'diagnoses-import-template.csv', 'text/csv');
    }).fail((error) => {
      console.log(error)
    });
  }
  ...
  render() {
    ...
    const modalOpts = {
      title: 'Import Custom Diagnosis codes',
      visible,
      footer: null,
      onCancel,
      width: 720,
    ...
    const props = {
      ...
      accept: '.csv',
      headers,
      action: `${API_URL + api.diagnosesImport}`,
      data: {},
    ...
    return (
      <Modal {...modalOpts}>
        <Alert
          description={(
            <div>
              To get started you'll need to format custom diagnosis codes in the correct way. Download a template CSV
