...
import { createContactPerson, queryContactPerson } from '../../services/accounting/contacts';
...
export default modelExtend(collectionModel, {
  ...
  effects: {
    *query({ payload }, { call, put }) {
      ...
      try {
        const data = yield call(queryContactPerson, parse(payload));
        if (data) {
          yield put({
            type: 'querySuccess',
            payload: {
    ...
    *create({ payload }, { call, put }) {
      ...
      try {
        const data = yield call(createContactPerson, payload);
        if (data) {
