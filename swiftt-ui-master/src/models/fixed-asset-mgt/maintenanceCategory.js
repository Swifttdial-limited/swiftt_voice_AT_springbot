...
import { queryOne, remove, update, applyAction } from '../../services/fixed-asset-mgt/maintenanceCategories';

export default modelExtend(resourceModel, {

  namespace: 'fixed_asset_maintenance_category',
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen((location) => {
        const match = pathToRegexp('/fixed-assets-management/maintenance/category/view/:id').exec(location.pathname);
        if (match) {
