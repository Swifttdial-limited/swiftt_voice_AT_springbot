...
/**
 * A Rest API should have a simple interface and a simple way for handling errors. This enumeration contains a subset
 * of HTTP status codes that should be used as part of an error response.
 *
 * Think hard about adding another status code to this class keeping things simple is good, especially for a front end
 * developer building a user interface why should they have to deal with numerous error conditions that boil down to its
 * the client's fault, its the server's fault.
 *
...
public enum RestApiHttpStatus {
    ...
    RestApiHttpStatus(int statusCode, String description) {
